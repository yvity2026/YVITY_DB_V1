          ...
          <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth overflow-x-visible">
            {visibleMenuItems.map((section, i) => (
              <div key={i} className="mb-5">
                {!collapsed && (
                  <h3 className="font-cormorant text-sm font-bold leading-2 tracking-[1.4px] px-6 text-left my-4 text-[#53807E] uppercase ">
                    {section.title}
                  </h3>
                )}
    ...
    `}
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="py-[18px] px-4 min-h-[60px] sticky top-0 left-0 right-0 z-10 flex items-center justify-between md:px-[90px] md:py-[10px] bg-white shadow-[0_0_4px_0_rgba(0,0,0,0.25)]">
          <h3 className="text-black text-base font-bold leading-normal font-cormorant">
            {currentHeader.title}
          </h3>
          ...
          {currentHeader.actions.length > 0 && (
            <div className="hidden h-10 md:flex gap-4  items-center">
              ...
              {currentHeader.actions.includes("profile") && (
                <motion.button className="rounded-full h-10 w-10 bg-[#F59E0B] cursor-pointer ring-[2px] ring-[#FEC564] overflow-hidden relative flex items-center justify-center">
                  {adminProfileImage ? (
                    ...
                    <p className="font-bold text-sm text-white">{adminInitials}</p>
                  )}
                </motion.button>
              )}
              ...
              {!(
                ...
                <motion.button
                  onClick={() => {
                    if (pathname === "/admin/services") openModal("ADD_SERVICE");
                    if (pathname === "/admin/professional-journey") openModal("ADD_ENTRY");
                    if (pathname === "/admin/achievements") openModal("ADD_ACHIEVEMENT");
                    if (pathname === "/admin/testimonials") openModal("REQUEST_TESTIMONIAL");
                    if (pathname === "/admin/gallery") openModal("ADD_PHOTO");
                    if (pathname === "/admin/professional-journey") openModal("ADD_PROFESSIONAL_JOURNEY");
                    if (pathname === "/admin/recommendations") openModal("ADD_RECOMMENDATION");
                  }}
              ...
              <div className="flex-1 h-screen overflow-y-auto">
                {visibleMenuItems.map((section, i) => (
                  <div key={i} className="mb-4">
                    <h3 className="font-cormorant text-xs tracking-widest px-6 mt-4 mb-2 text-[#53807E] uppercase">
                      {section.title}
                    </h3>
